...
if not os.path.exists(file_name) or os.stat(file_name).st_size == 0:
    prices.to_csv(file_name, index=True)
    print(f"📊 Archivo creado con {len(prices)} filas.")
else:
    df_existente = pd.read_csv(file_name, index_col="Date")
    df_existente.index = pd.to_datetime(df_existente.index)

    # Filtramos de prices solo las fechas que NO están ya en el CSV
    fechas_nuevas = prices.index.difference(df_existente.index)

    if len(fechas_nuevas) > 0:
        df_final = pd.concat([df_existente, prices.loc[fechas_nuevas]]).sort_index()
        df_final.to_csv(file_name)
        print(f"📈 {len(fechas_nuevas)} fechas nuevas agregadas: {fechas_nuevas.date.tolist()}")
    else:
        print("✅ El archivo ya está actualizado, no hay fechas nuevas.")
print("\n🏆 TOP 15")
